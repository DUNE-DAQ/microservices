...
getRunMeta = "select RUN_NUMBER, START_TIME, STOP_TIME, DETECTOR_ID, RUN_TYPE, SOFTWARE_VERSION from RUN_REGISTRY_META where RUN_NUMBER=%(run_num)s"

# $ curl -u fooUsr:barPass -X GET -O -J np04-srv-021:5005/runregistry/getRunBlob/2 
getRunBlob = "select FILENAME, CONFIGURATION from RUN_REGISTRY_META natural join RUN_REGISTRY_CONFIGS where RUN_NUMBER=%(run_num)s" 
...
insertRunRegistryMeta = "insert into RUN_REGISTRY_META (RUN_NUMBER, START_TIME, STOP_TIME, DETECTOR_ID, RUN_TYPE, FILENAME, SOFTWARE_VERSION) values (%(run_num)s, LOCALTIMESTAMP(6), NULL, %(det_id)s, %(run_type)s, %(filename)s, %(software_version)s)"
