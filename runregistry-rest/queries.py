...
getRunMeta = "select RUN_NUMBER, START_TIME, STOP_TIME, DETECTOR_ID, RUN_TYPE, COMMENT from RUN_REGISTRY_META where RUN_NUMBER=:run_num"
...
getRunMetaLast = "select * from (select RUN_NUMBER, START_TIME, STOP_TIME, DETECTOR_ID, RUN_TYPE, COMMENT from RUN_REGISTRY_META order by RUN_NUMBER desc) where ROWNUM<=:amount"
...
# $ curl -u fooUsr:barPass -F "file=@sspconf.tar.gz" -F "run_num=4" -F "det_id=foo" -F "run_type=bar" -F "comment=whatnot" -X POST http://localhost:5005/runregistry/insertRun/
insertRunRegistryMeta = "insert into RUN_REGISTRY_META (RUN_NUMBER, START_TIME, STOP_TIME, DETECTOR_ID, RUN_TYPE, FILENAME, COMMENT, SOFTWARE_VERSION) values (:run_num, CURRENT_TIMESTAMP, NULL, :det_id, :run_type, :filename, :comment, :software_version)"
