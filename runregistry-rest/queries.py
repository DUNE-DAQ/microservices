...
getRunMeta = "select RUN_NUMBER, START_TIME, STOP_TIME, DETECTOR_ID, RUN_TYPE, MESSAGE from RUN_REGISTRY_META where RUN_NUMBER=:run_num"
...
getRunMetaLast = "select * from (select RUN_NUMBER, START_TIME, STOP_TIME, DETECTOR_ID, RUN_TYPE, MESSAGE from RUN_REGISTRY_META order by RUN_NUMBER desc) where ROWNUM<=:amount"
...
# $ curl -u fooUsr:barPass -F "message=whatever" -F "run_num=2" -X POST np04-srv-021:5005/runregistry/updateExtraInfo/
updateMessage = "update RUN_REGISTRY_META set MESSAGE=:message where RUN_NUMBER=:run_num"
