...
# Orchestrator script to create partitions for all non-system, non-partition tables
# Usage:
...
while IFS= read -r table; do
    ...
    if SQL_OUTPUT=$("${PARTITION_SCRIPT}" "${table}" "${YEAR}" 2>&1); then
        if [[ -n "${SQL_OUTPUT}" ]]; then
            ...
            if ! echo "${SQL_OUTPUT}" | psql "${DATABASE_URI}" -v ON_ERROR_STOP=1 >/dev/null 2>&1; then
                add_error "Table '${table}': Failed to execute partition DDL"
            else
                echo "  Partitions created successfully" >&2
            fi
        fi
    else
        add_error "Table '${table}': Failed to generate partition DDL"
    fi
done <<<"${TABLES_OUTPUT}"
