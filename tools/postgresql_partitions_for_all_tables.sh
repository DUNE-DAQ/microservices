...
# Orchestrator script to create partitions for all non-system tables (both regular and parent partitioned tables)
# Usage:
#   DATABASE_URI="postgresql://user:pass@host/db" ./postgresql_partitions_for_all_tables.sh
# Or with specific year:
#   YEAR=2025 DATABASE_URI="postgresql://user:pass@host/db" ./postgresql_partitions_for_all_tables.sh
...
# Function to find all non-system tables (both regular and parent partitioned tables)
find_tables() {
    local query="SELECT n.nspname || '.' || c.relname AS table_name
FROM pg_class c
...
# Fetch table list
if ! TABLES_OUTPUT=$(find_tables | psql -XAtq -v ON_ERROR_STOP=1 "${DATABASE_URI}" 2>&1); then
    echo "Error: Failed to query tables from database" >&2
    if [[ -n "${TABLES_OUTPUT:-}" ]]; then
        echo "Error details: ${TABLES_OUTPUT}" >&2
    fi
    exit 1
fi
...
while IFS= read -r table; do
    ...
    if SQL_OUTPUT=$("${PARTITION_SCRIPT}" "${table}" "${YEAR}" 2>&1); then
        if [[ -n "${SQL_OUTPUT}" ]]; then
            echo "  Executing partition DDL..." >&2
            if ! PSQL_OUTPUT=$(printf '%s\n' "${SQL_OUTPUT}" | psql -X "${DATABASE_URI}" -v ON_ERROR_STOP=1 2>&1); then
                add_error "Table '${table}': Failed to execute partition DDL - ${PSQL_OUTPUT}"
            else
                echo "  Partitions created successfully" >&2
            fi
        fi
    else
        # SQL_OUTPUT contains the error message (stdout + stderr from failed run)
        add_error "Table '${table}': Failed to generate partition DDL - ${SQL_OUTPUT}"
    fi
done <<<"${TABLES_OUTPUT}"
